...
module.exports = function (app, config, passport) {
	...
	app.get("/setCFCookies*", ensureAuthenticated, async function(req, res, next){
		const cookies = await cfcookies.sign();
		const hash = cookies.hash;

		res.cookie("saml-logged", '1', {path: "/", secure: true, sameSite: 'strict', maxAge: cookies.expires});
		const authorized = await authorization.getPermissions(req.user.profile[config.saml_profile_uid]);
		res.cookie("saml-authorized", authorized ? '1' : '0', {path: "/", secure: true, sameSite: 'strict', maxAge: cookies.expires});
